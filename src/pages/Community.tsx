...
const CommunityPage = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900/10 to-emerald-900/10">
      <Navbar />
      ...
      {!selectedCategory ? <div className="container mx-auto px-4 py-32 bg-[#272727]">
        ...
        duration: 0.5
          ...
          delay: 0.6
            ...
            delay: 0.2 + index * 0.1
          }} className={`p-4 rounded-xl bg-white/5 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 ${selectedCategory === category.name ? 'ring-2 ring-blue-500' : ''}`} onClick={() => setSelectedCategory(category.name)}>
          ...
          delay: 0.8
              ...
              {communities.filter(community => community.name.toLowerCase().includes(searchTerm.toLowerCase()) || community.interests.some(interest => interest.toLowerCase().includes(searchTerm.toLowerCase()))).map((community, index) => <motion.div key={community.id || index} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
