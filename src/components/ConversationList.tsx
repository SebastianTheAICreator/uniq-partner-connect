...
import { MessageCircle, Sparkles, Filter, SlidersHorizontal, Search, Radio, TrendingUp, Clock, Users, Activity, LayoutGrid, MessageSquare, LayoutList, PlusCircle } from 'lucide-react';
...
const ConversationList = ({
  ...
  const handleNewDiscussion = async (discussion: {
    title: string;
    description: string;
  }) => {
    const newTopic = {
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    return conv.title.toLowerCase().includes(searchQuery.toLowerCase()) || conv.description && conv.description.toLowerCase().includes(searchQuery.toLowerCase());
  }).sort((a, b) => {
    if (sortBy === 'recent') {
  ...
  return <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-[#151822] via-[#1A1F2C] to-[#151822]">
      <Sidebar conversations={sidebarTopics} onConversationClick={handleTopicClick} />
      ...
      <div className="pl-[280px] sm:pl-[80px] transition-all duration-300">
        ...
        duration: 0.5
          ...
          <AnimatePresence mode="wait">
            {selectedTopic ? <motion.div key="topic-posts" initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            duration: 0.4
                ...
                <TopicPosts topicId={selectedTopic} topic={{
              title: selectedTopicData?.title || '',
              description: selectedTopicData?.description || ''
            }} onBack={() => setSelectedTopic(null)} />
              </motion.div> : <motion.div key="topic-list" initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }} exit={{
            ...
            duration: 0.4
                ...
                <header className="px-8 pt-16 pb-10">
                  <div className="max-w-7xl mx-auto">
                    ...
                    top: document.getElementById('create-discussion')?.offsetTop,
                    behavior: 'smooth'
                    ...
                    <div className="mt-10 p-1.5 rounded-2xl bg-white/5 backdrop-blur-md border border-white/5 flex flex-col sm:flex-row items-center gap-3">
                      <div className="relative flex-1 w-full">
                        ...
                        <Input type="search" placeholder="Caută conversații..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-white/5 border-0 pl-10 py-6 text-white placeholder:text-white/40 focus:ring-1 focus:ring-primary/30" />
                      </div>
                      
                      <div className="flex items-center gap-3 px-2 w-full sm:w-auto">
                        <Button variant="ghost" size="sm" className={cn("rounded-xl px-3 py-2 h-10 text-white/60", sortBy === 'recent' && "bg-white/10 text-white")} onClick={() => setSortBy('recent')}>
                          <Clock className="h-4 w-4 mr-2" />
                          Recente
                        </Button>
                        
                        <Button variant="ghost" size="sm" className={cn("rounded-xl px-3 py-2 h-10 text-white/60", sortBy === 'popular' && "bg-white/10 text-white")} onClick={() => setSortBy('popular')}>
                          <Users className="h-4 w-4 mr-2" />
                          Populare
                        </Button>
                        
                        <Button variant="ghost" size="sm" className={cn("rounded-xl px-3 py-2 h-10 text-white/60", sortBy === 'trending' && "bg-white/10 text-white")} onClick={() => setSortBy('trending')}>
                        ...
                        <Button variant="ghost" size="sm" className={cn("rounded-xl w-10 h-10 p-0 flex items-center justify-center", viewMode === 'grid' && "bg-white/10 text-white")} onClick={() => setViewMode('grid')}>
                          <LayoutGrid className="h-4 w-4" />
                        </Button>
                        
                        <Button variant="ghost" size="sm" className={cn("rounded-xl w-10 h-10 p-0 flex items-center justify-center", viewMode === 'list' && "bg-white/10 text-white")} onClick={() => setViewMode('list')}>
                          <LayoutList className="h-4 w-4" />
                        </Button>
                        
                        <Button variant="ghost" size="sm" className={cn("rounded-xl w-10 h-10 p-0 flex items-center justify-center", filterActive && "bg-primary/20 text-primary")} onClick={() => setFilterActive(!filterActive)}>
                          <SlidersHorizontal className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>
                    
                    {filterActive && <motion.div initial={{
                  opacity: 0,
                  height: 0
                }} animate={{
                  opacity: 1,
                ...
                <div className="py-4 bg-white/5 backdrop-blur-sm px-0">
                ...
                <main className="px-8 py-12">
                  <div className="max-w-7xl mx-auto">
                    {filteredConversations.length > 0 ? <div className={cn("grid gap-8", viewMode === 'grid' ? "grid-cols-1 md:grid-cols-2 xl:grid-cols-3" : "grid-cols-1")}>
                        {filteredConversations.map(conv => <TopicCard key={conv.id} topic={{
                    id: conv.id?.toString() || '',
                    title: conv.title,
                    ...
                    tags: ['Design', 'UI/UX', 'Technology', 'Web Dev'].slice(0, Math.floor(Math.random() * 3) + 1)
                        ...
                        <p className="text-white/50">
                          {searchQuery ? `Nu există conversații care să conțină "${searchQuery}"` : "Nu există conversații active momentan"}
                        </p>
                      </div>}
                  </div>
                </main>
